...
-- Fast Thread
RegisterNetEvent('lowkey_sit:startLoop')
AddEventHandler('lowkey_sit:startLoop', function()
    ...
    CreateThread(function()
        while InUse do
            ...
            if IsDisabledControlJustPressed(0, Config.objects.ButtonToStandUp) or (#(plyCoords) - #(PlyLastPos)) > 5 then
                ...
                InUse = false
                TriggerServerEvent('lowkey_sit:Leave', oElement.fObjectCoords)
...
RegisterNetEvent('lowkey_sit:Animation')
AddEventHandler('lowkey_sit:Animation', function(v, coords)
    ...
    TriggerEvent('lowkey_sit:startLoop')
end)
...
RegisterNetEvent('lowkey_sit:sit')
AddEventHandler('lowkey_sit:sit', function(data)
    ...
    for _, element in pairs(Config.objects.locations) do
        if GetHashKey(element.object) == GetEntityModel(data.entity) then
            ...
            TriggerServerEvent('lowkey_sit:Enter', oElement, oElement.fObjectCoords)
            break
        end
    end
end)
